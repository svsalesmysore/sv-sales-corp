...
function ResultRow({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-brand-red/30 hover:shadow-sm transition-all bg-white">
      <Link href={`/products/${product.categoryId}/${product.id}`} onClick={onClose} className="relative w-14 h-14 rounded-xl overflow-hidden bg-white border border-slate-100 shrink-0 cursor-pointer">
        <Image src={img} alt={product.name} fill sizes="56px" className="object-contain p-1" onError={() => setImgErr(true)} unoptimized />
      </Link>
      ...
      <button
        ...
        className={cn(
          ...
          added ? 'bg-green-50 text-green-600 border border-green-200'
                : needsSize ? 'bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed'
...
export default function SearchOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-brand-dark/70 backdrop-blur-md animate-fade-in [animation-duration:200ms]" onClick={onClose}>
      <div className="bg-white w-full max-w-3xl mx-auto mt-0 sm:mt-20 rounded-none sm:rounded-3xl shadow-2xl shadow-black/40 ring-1 ring-white/10 overflow-hidden flex flex-col max-h-screen sm:max-h-[80vh] animate-fade-up [animation-duration:300ms]" onClick={(e) => e.stopPropagation()}>
        {/* search input */}
